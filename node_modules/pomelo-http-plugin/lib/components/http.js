...
var Http = function(app, opts) {
  ...
  //this.http.use(createExpressLogger(this.logger));               //express的日志加载
  ...
  //xj添加的相关配置信息
  this.http.set('views', path.join(app.getBase(), opts.views));    //页面模板的位置
  this.http.set('view engine', 'ejs');                             //设置ejs引擎
  this.http.use(flash());
  this.http.use(express.cookieParser());                           //支持cookie解析
  this.http.use(express.session({                                  //支持session
      secret: 'admin123'
  }));
  ...
  this.http.use(this.http.router);                                        //加载路由
  this.http.use(express.static(path.join(app.getBase(), opts.public)));   //设置静态文件地址
  //xj配置结束
...
Http.prototype.loadRoutes = function() {
  this.http.get('/test', function(req, res) {
