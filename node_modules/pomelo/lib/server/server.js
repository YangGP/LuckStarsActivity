...
var Server = function (app, opts) {
  this.opts = opts || {};
  this.app = app;
  this.globalFilterService = null;    //全局服务器过滤函数
  this.filterService = null;          //局部过滤函数
  this.handlerService = null;         //保存handlerService对象
  this.crons = [];                    //定时器数组
  this.jobs = {};                     //定时任务数组
  this.state = ST_INITED;

  app.event.on(events.ADD_CRONS, this.addCrons.bind(this));        //为app添加add_crons事件
  app.event.on(events.REMOVE_CRONS, this.removeCrons.bind(this));  //为app添加remove_crons事件
};
...
pro.start = function() {
  ...
  //创建common/service/filterService对象，加载app全局filter(app._golbalBefores_,app._golbalAfters_)
  this.globalFilterService = initFilter(true, this.app);
  //创建common/service/filterService对象，加载app局部filter(app._Befores_,app._Afters_)
  this.filterService = initFilter(false, this.app);
  //创建common/service/handlerService对象
  this.handlerService = initHandler(this.app, this.opts);
  //加载server目录下的cron文件
  this.cronHandlers = loadCronHandlers(this.app);
  //加载config/crons.json定义的crons保存到crons数组
...
var loadCrons = function(server, app) {
  ...
  app.loadConfigBaseApp(Constants.RESERVED.CRONS, Constants.FILEPATH.CRON); //加载config/crons.json文件
  var crons = app.get(Constants.RESERVED.CRONS);         //读取config/crons.json
  for(var serverType in crons) {                         //遍历所有的crons，并保存到crons数组中
