...
pro.start = function(cb) {
  this.opts.port = this.app.getCurServer().port;    //当前服务器的端口
  this.remote = genRemote(this.app, this.opts);     //创建rpc server对象
...
var genRemote = function(app, opts) {
  ...
  opts.context = app;
  if(!!opts.rpcServer) {
    return opts.rpcServer.create(opts);   //创建自定义的rpc server对象
  } else {
    return RemoteServer.create(opts);     //创建rpc server对象
  }
};
