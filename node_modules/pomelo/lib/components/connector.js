...
var Component = function(app, opts) {
  opts = opts || {};
  this.app = app;
  //获取connector，如果server没有配置connectorConfig的connector信息，那么使用默认的/connectors/sioconnector.js作为连接器
  this.connector = getConnector(app, opts);
  this.encode = opts.encode;             //保留用户自定义编码函数，否则使用connector的encode
  this.decode = opts.decode;             //保留用户自定义解码函数，否则使用connector的decode
  this.useCrypto = opts.useCrypto;       //消息是否加密
  this.blacklistFun = opts.blacklistFun; //连接的黑名单过滤函数
  this.keys = {};                        //加密密钥
  this.blacklist = [];                   //黑名单数组
...
pro.start = function(cb) {
  ...
  this.session = this.app.components.__session__;        //获取app中的组件,单例模式一个app中只有一个
...
/**
 * 启动连接器，并绑定connection事件，监听客户端的连接
 * @param cb
 */
pro.afterStart = function(cb) {
  this.connector.start(cb);     //启动连接器，当前我们连接的是hybridconnector，所以启动它
  this.connector.on('connection', hostFilter.bind(this, bindEvents));  //此处新客户端到来
};
...
pro.send = function(reqId, route, msg, recvs, opts, cb) {
  ...
  if(this.encode) {
    ...
    emsg = this.encode.call(this, reqId, route, msg);   //使用用户自定义的编码
  } else if(this.connector.encode) {
    // use connector default encode
    emsg = this.connector.encode(reqId, route, msg);    //使用连接器自带的编码
  }
...
/**
 * 获取connector，如果server没有配置connectorConfig的connector信息，
 * 那么使用默认的/connectors/sioconnector.js作为连接器。
 * @param app
 * @param opts
 * @returns {*}
 */
...
var hostFilter = function(cb, socket) {
  ...
  // dynamical check 动态检查：该socket存在黑名单里，立即返回
  ...
  if(!!this.blacklistFun && typeof this.blacklistFun === 'function') {
    ...
    utils.invokeCallback(cb, this, socket);  //通过过滤，执行bindEvents绑定功能
  }
};

var bindEvents = function(self, socket) {   //socket是hybridsocket对象，在此会创建session
  if(self.connection) {
