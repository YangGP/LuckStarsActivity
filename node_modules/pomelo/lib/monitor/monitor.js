...
var Monitor = function(app, opts) {
  opts = opts || {};
  this.app = app;
  this.serverInfo = app.getCurServer();    //保存当前服务器信息
  this.masterInfo = app.getMaster();       //保存master服务器信息
  this.modules = [];                       //保存已经加载的模块
  this.closeWatcher = opts.closeWatcher;   //是否关闭monitor监控功能

  this.monitorConsole = admin.createMonitorConsole({  //创建pomelo-admin的monitor管理对象
