...
module.exports.defaultConfiguration = function(app) {
  ...
  setupEnv(app, args);       //设置环境变量为development还是product
  loadMaster(app);           //读取./config/master.json中的master服务器信息
  loadServers(app);          //读取./config/server.json中的server服务器信息
  processArgs(app, args);    //解析命令行参数,并且为serverType、serverId等赋值
  configLogger(app);         //设置日志./config/log4js.json中的配置
  loadLifecycle(app);        //加载进程的生命文件，后面用到的services中的beforeStartup等
};
 ...
 * 加载默认组件，并存入app.components中
 * master拥有组件：master、monitor
 * 前端特有组件：connection、connector、session、pushSchedule
 * 后端特有组件：remote
 * 前后端共有组件:proxy,backendSession,channel,server,monitor
 */
...
module.exports.optComponents = function(comps, method, cb) {
  ...
  async.forEachSeries(comps, function(comp, done) {
    ...
    if (typeof comp[method] === 'function') {
      comp[method](done);   //调用组件的start函数，启动该组件
    } else {
      done();
    }
  }, function(err) {
