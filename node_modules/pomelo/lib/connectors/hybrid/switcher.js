...
var Switcher = function(server, opts) {
  ...
  this.server = server;                                     //hybridconnector.listeningServer
  this.wsprocessor = new WSProcessor();                     //关联./weprocessor.js
  this.tcpprocessor = new TCPProcessor(opts.closeMethod);   //关联./tcpprocesor.js
  this.id = 1;                                              //默认客户端socket的id
  this.timeout = (opts.timeout || DEFAULT_TIMEOUT) * 1000;  //超时
  this.setNoDelay = opts.setNoDelay;                        //是否设置setNoDelay参数

  //当有客户端连接的时候进入newSocket函数,由server在emit时，传递的socket给newSocket使用
  if (!opts.ssl) {
