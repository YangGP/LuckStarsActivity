 ...
 * Get channel by name.
 * 获取一个频道：
 * 根据create的标志。为true，那么没有该频道时，创建新的频道并维护该频道信息
 * @param {String} name channel's name
...
var Channel = function(name, service) {
  this.name = name;
  this.groups = {};       // fserverId:[uid,uid,uid]         sid这个前端服务器上连接的用户id列表
  this.records = {};      // uid:{sid: fserverId, uid: uid}  用户对应的前端连接信息
 ...
 * @param {String} sid frontend server id which user has connected to 用户连接的前端服务器的id
 */
...
/**
 * 读取本地缓存里先前游戏服务器的channel数据，然后恢复这些数据，保证channel不会丢失
 * @param self
 * @param cb
 */
...
var genKey = function(self, name) {
  if(!!name) {
    return self.prefix + ':' + self.app.serverId + ':' + name;    //value
  } else {
    return self.prefix + ':' + self.app.serverId;                 //key
  }
};
