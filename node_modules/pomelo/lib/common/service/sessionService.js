...
var SessionService = function(opts) {
  opts = opts || {};
  this.singleSession = opts.singleSession;        //是否是单session用户
  this.sessions = {};     // sid -> session       //存放所有客户端sessions
  this.uidMap = {};       // uid -> sessions      //存放已经绑定过uid的sessions
};
...
SessionService.prototype.create = function(sid, frontendId, socket) {
  var session = new Session(sid, frontendId, socket, this);   //创建session对象
  this.sessions[session.id] = session;                        //将创建的session对象保存到sessions中
...
SessionService.prototype.kickBySessionId = function(sid, reason, cb) {
  ...
  if(session) {
    // notify client
    session.closed(reason);         //在session.close()中，会去处理sessions和uidMap
    process.nextTick(function() {
      utils.invokeCallback(cb);
    });
  } else {
...
var Session = function(sid, frontendId, socket, service) {
  ...
  this.uid = null;        // r
  this.settings = {};                      //保存自定义的session数据

  // private
  this.__socket__ = socket;                //保存原生的hybridsocket对象
  this.__sessionService__ = service;       //sessionService
...
var FrontendSession = function(session) {
  EventEmitter.call(this);
  clone(session, this, FRONTEND_SESSION_FIELDS);   //克隆session类的属性
